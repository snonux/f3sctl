// Package contract is the REST vocabulary shared by the API's composition
// root (internal/httpapi) and its per-domain surfaces (gogiosapi, powerapi).
//
// It is deliberately dumb: Route, State, Request and the Siren rendering
// types here are pure data and small helpers, with no behaviour of their own
// and no imports of the composition root. The direction of knowledge is one
// way -- the root and both surface packages know the contract, the contract
// knows neither of them -- which is what lets the Gogios and power surfaces
// live in packages of their own without an import cycle, and what makes any
...
type Action struct {
	...
	// (see Route), so the CLI-command-to-action-name mapping is declared
